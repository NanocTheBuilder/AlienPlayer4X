...
public class ShipSizeBuyer extends TechBuyer {
    private static int[] SHIP_SIZE_ROLL_TABLE = new int[]{0, 10, 7, 6, 5, 3, 6};

    public ShipSizeBuyer(Game game){
        super(game, Technology.SHIP_SIZE);
    }

    @Override
    protected boolean isNeeded(Fleet fleet, FleetBuildOption... options) {
        AlienPlayer ap = fleet.getAp();
        return ap.getLevel(SHIP_SIZE) < game.scenario.getMaxLevel(SHIP_SIZE)
            && game.roller.roll() <= SHIP_SIZE_ROLL_TABLE[ap.getLevel(SHIP_SIZE)];
    }
